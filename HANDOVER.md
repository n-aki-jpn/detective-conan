...
### 問題14: モバイルでのカードタップ時にメニューが表示されない
**原因**:
- モバイル環境（タッチデバイス）において、`dblclick`と`click`イベントの競合により、カードをタップしてもコンテキストメニューが表示されない問題が発生していました。
- `attachCardEvents` 関数内で、`area === "field"` または `area === "partner"` のカードに対してのみコンテキストメニューを開くという条件が、タップイベントでも適用されていたため、手札のカードではメニューが表示されていませんでした。

**修正**:
- `app.js` の `attachCardEvents` 関数を修正し、モバイルデバイス（`window.matchMedia("(hover: none) and (pointer: coarse)").matches` で判定）の場合、すべてのカード（手札、フィールド、パートナー）で `click` イベント時に `openContextMenu` が呼ばれるように修正しました。
- また、フィールド上のアクティブなキャラクターカードをモバイルでタップした際に、コンテキストメニューに加えてアクションターゲットメニューも直接開くように変更し、操作フローを改善しました。

### 問題15: カットイン時にキャラクターカードが選択できない
**原因**:
- `state.js` の `useCutIn` および `useDisguise` 関数において、カットインや変装の対象となるキャラクターを特定するロジックが不十分でした。
- `useCutIn` 関数では、攻撃者と防御者の両方からカットイン対象のキャラを見つけようとしていましたが、`this.guardPhase.attackerId` と `this.guardPhase.defenderId` はコンタクト中のキャラのIDであり、カットインを使用しているプレイヤー側のキャラIDのみを参照すべきでした。また、見つからなかった場合のエラー処理も不完全でした。
- `useDisguise` 関数でも同様に、変装元のキャラを `this.guardPhase.attackerId` または `this.guardPhase.defenderId` から探す際に、それが変装を使用するプレイヤー自身のフィールド上のキャラであるかの検証が不足していました。

**修正**:
- `state.js` の `useCutIn` 関数を修正し、カットインを使用するプレイヤー(`playerIndex`)に応じて、コンタクト中の攻撃キャラ(`this.guardPhase.attackerId`)または防御キャラ(`this.guardPhase.defenderId`)の中から、**そのプレイヤー自身のキャラ**を `targetChar` として正しく特定するように変更しました。
- `targetChar` が見つからなかった場合、使用したカットインカードを手札に戻すエラーハンドリングを追加しました。
- `state.js` の `useDisguise` 関数も修正し、変装元のキャラ(`originalCharLoc`)が変装を使用するプレイヤー自身のフィールドにいることを明確に検証するようにしました。
- 両関数で、`this.triggerStateChange()` を呼び出して UI を適切に更新するように変更しました。

### 問題16: ガードフェーズ時にキャラクターカードが選択できない
**原因**:
- `app.js` の `openContextMenu` 関数内で、ガードフェーズ中にカードをタップした際の処理が、アクティブなフィールド上のキャラのみをガード対象としていました。
- しかし、この条件が、プレイヤー自身のパートナーカードがガード対象となるケースを考慮していなかったため、パートナーカードが選択肢に表示されず、ガードができない状況となっていました。
- モバイル環境（タッチデバイス）において、`openContextMenu` でコンテキストメニューが表示された後、自動的に `openActionTargetMenu` が呼ばれる条件が設定されていましたが、ガードフェーズではこの自動起動が不要であり、意図しない挙動を引き起こしていました。

**修正**:
- `app.js` の `openContextMenu` 関数内のガードフェーズ処理を修正しました。
- ガード可能な条件を `(area === "field" || area === "partner") && playerIndex === activePIdx && card.state === "active" && card.type === "Character"` に変更し、アクティブな状態のキャラクタータイプを持つフィールド上のカードとパートナーカードの両方をガード対象として選択できるようにしました。
- モバイル環境でコンテキストメニューが表示された後、アクションターゲットメニューを自動的に開くロジックから、ガードフェーズ中の条件を削除しました。これにより、ガードフェーズ中はコンテキストメニューのみが表示され、ユーザーが手動でガードアクションを選択できるようになります。
